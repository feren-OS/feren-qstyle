...
void Style::polish(QWidget *widget)
{
    ...
    if (QAbstractItemView *itemView = qobject_cast<QAbstractItemView *>(widget)) {
        ...
        if (!Feren::Config::TitleWidgetDrawFrame) {
...
void Style::polishScrollArea(QAbstractScrollArea *scrollArea)
{
    ...
    if (scrollArea->property(PropertyNames::sidePanelView).toBool()) {
        ...
        if (!Feren::Config::SidePanelDrawFrame) {
...
int Style::styleHint(StyleHint hint, const QStyleOption *option, const QWidget *widget, QStyleHintReturn *returnData) const
{
    ...
    case SH_Widget_Animate:
        return Feren::Config::AnimationsEnabled;
    ...
    case SH_TabBar_Alignment:
        return Feren::Config::TabBarDrawCenteredTabs ? Qt::AlignCenter : Qt::AlignLeft;
...
void Style::loadConfiguration()
{
    ...
    // mnemonics
    _mnemonics->setMode(Feren::Config::MnemonicsMode);

    // splitter proxy
    _splitterFactory->setEnabled(Feren::Config::SplitterProxyEnabled);

    // clear icon cache
    _iconCache.clear();

    // scrollbar buttons
    switch (Feren::Config::ScrollBarAddLineButtons) {
    case 0:
    ...
    switch (Feren::Config::ScrollBarSubLineButtons) {
    case 0:
    ...
    // frame focus
    if (Feren::Config::ViewDrawFocusIndicator)
...
QSize Style::sliderSizeFromContents(const QStyleOption *option, const QSize &contentsSize, const QWidget *) const
{
    ...
    bool horizontal(sliderOption->orientation == Qt::Horizontal);
    bool disableTicks(!Feren::Config::SliderDrawTickMarks);
...
bool Style::drawFramePrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    const bool isTitleWidget(Feren::Config::TitleWidgetDrawFrame && widget && widget->parent() && widget->parent()->inherits("KTitleWidget"));
    // copy state
    const State &state(option->state);
    ...
    // render
    if (!Feren::Config::SidePanelDrawFrame && widget && widget->property(PropertyNames::sidePanelView).toBool()) {
...
bool Style::drawIndicatorHeaderArrowPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *) const
{
    ...
    if (Feren::Config::ViewInvertSortIndicator)
...
bool Style::drawIndicatorToolBarHandlePrimitive(const QStyleOption *option, QPainter *painter, const QWidget *) const
{
    ...
    if (!Feren::Config::ToolBarDrawItemSeparator)
        return true;
...
bool Style::drawIndicatorToolBarSeparatorPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
     ...
     */
    if (!(Feren::Config::ToolBarDrawItemSeparator || qobject_cast<const QComboBox *>(widget))) {
        return true;
    }
...
bool Style::drawIndicatorBranchPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *) const
{
    ...
    // tree branches
    if (!Feren::Config::ViewDrawTreeBranchLines)
        return true;
...
bool Style::drawSliderComplexControl(const QStyleOptionComplex *option, QPainter *painter, const QWidget *widget) const
{
    ...
    // tickmarks
    if (Feren::Config::SliderDrawTickMarks && (sliderOption->subControls & SC_SliderTickmarks)) {
...
bool Style::drawScrollBarComplexControl(const QStyleOptionComplex *option, QPainter *painter, const QWidget *widget) const
{
    ...
    bool animated(Feren::Config::ScrollBarShowOnMouseOver && _animations->scrollBarEngine().isAnimated(widget,  AnimationHover, QStyle::SC_ScrollBarGroove));
...
QColor Style::scrollBarArrowColor(const QStyleOptionSlider *option, const SubControl &control, const QWidget *widget) const
{
    ...
    bool enabled(option->state & State_Enabled);
    if (!enabled) {
        if (Feren::Config::ScrollBarShowOnMouseOver) {
...
QIcon Style::titleBarButtonIcon(StandardPixmap standardPixmap, const QStyleOption *option, const QWidget *widget) const
{
    ...
    bool isCloseButton(buttonType == ButtonClose && Feren::Config::OutlineCloseButton);
...
bool Style::hasAlteredBackground(const QWidget *widget) const
{
    ...
    else if (Feren::Config::DockWidgetDrawFrame && qobject_cast<const QDockWidget *>(widget))
        hasAlteredBackground = true;
