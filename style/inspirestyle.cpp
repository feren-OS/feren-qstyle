...
#include "inspire.h"
#include "inspireanimations.h"
#include "inspirehelper.h"
#include "inspiremnemonics.h"
#include "inspiresplitterproxy.h"
#include "inspirewindowmanager.h"
#include "inspireblurhelper.h"
...
private:
    ...
    Inspire::WeakPointer<const QWidget> _tabBar;
...
public:
    ...
    ComboBoxItemDelegate(QAbstractItemView *parent)
        ...
        , _itemMargin(Inspire::Metrics::ItemView_ItemMarginWidth)
    {}
...
private:
    //* proxy
    Inspire::WeakPointer<QAbstractItemDelegate> _proxy;
...
Style::Style()
    ...
    , _tabBarData(new InspirePrivate::TabBarData(this))
#if INSPIRE_HAVE_KSTYLE
...
void Style::polish(QWidget *widget)
{
    ...
    if (QAbstractItemView *itemView = qobject_cast<QAbstractItemView *>(widget)) {
        ...
        if (!Inspire::Config::TitleWidgetDrawFrame) {
    ...
    if (qobject_cast<QScrollBar *>(widget)) {
        ...
        if (!hasParent(widget, "QWebView")) {
            ...
            if (itemView && itemView->itemDelegate() && itemView->itemDelegate()->inherits("QComboBoxDelegate")) {
                itemView->setItemDelegate(new InspirePrivate::ComboBoxItemDelegate(itemView));
            }
...
void Style::polishScrollArea(QAbstractScrollArea *scrollArea)
{
    ...
    if (scrollArea->property(PropertyNames::sidePanelView).toBool()) {
        ...
        if (!Inspire::Config::SidePanelDrawFrame) {
...
int Style::styleHint(StyleHint hint, const QStyleOption *option, const QWidget *widget, QStyleHintReturn *returnData) const
{
    ...
    case SH_Widget_Animate:
        return Inspire::Config::AnimationsEnabled;
    ...
    case SH_TabBar_Alignment:
        return Inspire::Config::TabBarDrawCenteredTabs ? Qt::AlignCenter : Qt::AlignLeft;
...
bool Style::eventFilterDockWidget(QDockWidget *dockWidget, QEvent *event)
{
    if (event->type() == QEvent::Paint) {
        ...
        if (dockWidget->isFloating()) {
            _helper->renderMenuFrame(&painter, rect, background, outline, palette);
        } else if (Inspire::Config::DockWidgetDrawFrame || (dockWidget->features() & QDockWidget::AllDockWidgetFeatures)) {
...
void Style::loadConfiguration()
{
    ...
    // mnemonics
    _mnemonics->setMode(Inspire::Config::MnemonicsMode);

    // splitter proxy
    _splitterFactory->setEnabled(Inspire::Config::SplitterProxyEnabled);

    // clear icon cache
    _iconCache.clear();

    // scrollbar buttons
    switch (Inspire::Config::ScrollBarAddLineButtons) {
    case 0:
    ...
    switch (Inspire::Config::ScrollBarSubLineButtons) {
    case 0:
    ...
    // frame focus
    if (Inspire::Config::ViewDrawFocusIndicator)
...
QSize Style::sliderSizeFromContents(const QStyleOption *option, const QSize &contentsSize, const QWidget *) const
{
    ...
    bool horizontal(sliderOption->orientation == Qt::Horizontal);
    bool disableTicks(!Inspire::Config::SliderDrawTickMarks);
...
bool Style::drawFramePrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    const bool isTitleWidget(Inspire::Config::TitleWidgetDrawFrame && widget && widget->parent() && widget->parent()->inherits("KTitleWidget"));
    // copy state
    const State &state(option->state);
    ...
    // render
    if (!Inspire::Config::SidePanelDrawFrame && widget && widget->property(PropertyNames::sidePanelView).toBool()) {
...
bool Style::drawIndicatorHeaderArrowPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *) const
{
    ...
    if (Inspire::Config::ViewInvertSortIndicator)
...
bool Style::drawIndicatorToolBarHandlePrimitive(const QStyleOption *option, QPainter *painter, const QWidget *) const
{
    ...
    if (!Inspire::Config::ToolBarDrawItemSeparator)
        return true;
...
bool Style::drawIndicatorToolBarSeparatorPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
     ...
     */
    if (!(Inspire::Config::ToolBarDrawItemSeparator || qobject_cast<const QComboBox *>(widget))) {
        return true;
    }
...
bool Style::drawIndicatorBranchPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *) const
{
    ...
    // tree branches
    if (!Inspire::Config::ViewDrawTreeBranchLines)
        return true;
...
bool Style::drawMenuBarItemControl(const QStyleOption *option, QPainter *painter, const QWidget *) const
{
    ...
    bool sunken(enabled && (state & State_Sunken));
    bool useStrongFocus(Inspire::Config::MenuItemDrawStrongFocus);
...
bool Style::drawMenuItemControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (menuItemOption->checkType == QStyleOptionMenuItem::NonExclusive) {
        ...
        */
        ...
        _helper->renderCheckBox(painter, checkBoxRect, indicatorBackground, outline, tickColor, false, checkState, mouseOver, palette, mode, enabled && windowActive, true);
        ...
        */
        ...
        qreal opacity(_animations->widgetStateEngine().opacity(widget, AnimationHover));
        QColor tickColor = _helper->checkBoxIndicatorColor(palette, mouseOver, enabled && active, opacity, mode, true);
        _helper->renderRadioButton(painter, checkBoxRect, indicatorBackground, outline, tickColor, false, enabled && windowActive, active ? RadioOn : RadioOff, palette, mode, false, true);
    }
...
bool Style::drawSliderComplexControl(const QStyleOptionComplex *option, QPainter *painter, const QWidget *widget) const
{
    ...
    // tickmarks
    if (Inspire::Config::SliderDrawTickMarks && (sliderOption->subControls & SC_SliderTickmarks)) {
...
bool Style::drawScrollBarComplexControl(const QStyleOptionComplex *option, QPainter *painter, const QWidget *widget) const
{
    ...
    bool animated(Inspire::Config::ScrollBarShowOnMouseOver && _animations->scrollBarEngine().isAnimated(widget,  AnimationHover, QStyle::SC_ScrollBarGroove));
...
QColor Style::scrollBarArrowColor(const QStyleOptionSlider *option, const SubControl &control, const QWidget *widget) const
{
    ...
    bool enabled(option->state & State_Enabled);
    if (!enabled) {
        if (Inspire::Config::ScrollBarShowOnMouseOver) {
...
QIcon Style::titleBarButtonIcon(StandardPixmap standardPixmap, const QStyleOption *option, const QWidget *widget) const
{
    ...
    bool isCloseButton(buttonType == ButtonClose && Inspire::Config::OutlineCloseButton);
...
bool Style::hasAlteredBackground(const QWidget *widget) const
{
    ...
    else if (Inspire::Config::DockWidgetDrawFrame && qobject_cast<const QDockWidget *>(widget))
        hasAlteredBackground = true;
